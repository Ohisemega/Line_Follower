...
volatile long right_wheel_count; // used by encoder to count the rotation
volatile byte right_wheel_state;
volatile long left_wheel_count;
volatile byte left_wheel_state;
...
ISR( INT6_vect ) {
  ...
  // State:  new B   new A   old B   old A
    right_wheel_state = right_wheel_state | ( e0_B  << 3 );
    right_wheel_state = right_wheel_state | ( e0_A  << 2 );

    // Handle which transition we have registered.
    // Complete this if statement as necessary.
    // Refer to the labsheet.
//    Serial.println(right_wheel_state);
    if( right_wheel_state == 1 || right_wheel_state == 7 || right_wheel_state == 8 || right_wheel_state == 14 ) {
        right_wheel_count--;
    } else if( right_wheel_state == 2 || right_wheel_state == 4 || right_wheel_state == 11 || right_wheel_state == 13 ) {
        right_wheel_count++;
    }
...
//    Serial.println(right_wheel_count);
    ...
    right_wheel_state = right_wheel_state >> 2;
...
ISR( PCINT0_vect ) {
    ...
    left_wheel_state = left_wheel_state | ( e1_B  << 3 );
    left_wheel_state = left_wheel_state | ( e1_A  << 2 );


    // Handle which transition we have registered.
    // Complete this if statement as necessary.
    // Refer to the labsheet. 
    if( left_wheel_state == 1 || left_wheel_state == 7 || left_wheel_state == 8 || left_wheel_state == 14 ) {
        left_wheel_count--;
    } else if( left_wheel_state == 2 || left_wheel_state == 4 || left_wheel_state == 11 || left_wheel_state == 13 ) {
        left_wheel_count++;
    }
//    Serial.print("Left Wheel count ");
//    Serial.println(left_wheel_count);

    // Shift the current readings (bits 3 and 2) down
    // into position 1 and 0 (to become prior readings)
    // This bumps bits 1 and 0 off to the right, "deleting"
    // them for the next ISR call. 
    left_wheel_state = left_wheel_state >> 2;
}
...
void setupEncoder0() 
{
    ...
    right_wheel_state = right_wheel_state | ( e0_B << 1 );
    right_wheel_state = right_wheel_state | ( e0_B << 0 );
